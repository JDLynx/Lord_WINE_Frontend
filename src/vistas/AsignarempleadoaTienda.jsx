...
export default function AsignarEmpleadoaTienda() {
    ...
    const handleStoreChange = async (employeeId, newStoreId) => {
        // Actualizar el estado local para una respuesta visual inmediata
        const updatedEmployees = employees.map(emp => {
            if (emp.emplCodEmpleado === employeeId) {
                // Si newStoreId es una cadena vacía, asignamos null
                const newTiendId = newStoreId ? parseInt(newStoreId) : null;
                return {
                    ...emp,
                    tiendIdTiendaFisica: newTiendId,
                    // Actualizar el objeto anidado `tiendaFisica`
                    tiendaFisica: newStoreId ? stores.find(s => s.tiendIdTiendaFisica === parseInt(newStoreId)) : null
                };
            }
            return emp;
        });
        ...
        try {
            const res = await fetch(`https://lord-wine-backend.onrender.com/api/empleados/${employeeId}`, {
                ...
                body: JSON.stringify({ tiendIdTiendaFisica: newStoreId || null }),
            });
            ...
            // Opcional: Re-fetch de todos los empleados para garantizar la sincronización
            // fetchEmpleados();
                        ...
                        <table className="min-w-full bg-white text-black">
                            <thead className="bg-gray-100 sticky top-0">
                                <tr>
                                    ...
                                    <th className="px-4 py-2 text-left text-lg font-medium text-black">Tienda Asignada</th>
                                    <th className="px-4 py-2 text-left text-lg font-medium text-black">Dirección Tienda</th>
                                    <th className="px-4 py-2 text-left text-lg font-medium text-black">Teléfono Tienda</th>
                                    <th className="px-4 py-2 text-left text-lg font-medium text-black">Asignar Tienda</th>
                                </tr>
                            </thead>
                            <tbody>
                                {filtered.map(emp => (
                                    <tr key={emp.emplCodEmpleado}>
                                        ...
                                        {/* Mostrar datos de la tienda anidada */}
                                        <td className="px-4 py-2 text-left text-base">{emp.tiendaFisica?.tiendNombre || 'N/A'}</td>
                                        <td className="px-4 py-2 text-left break-words max-w-[250px] whitespace-normal text-base">{emp.tiendaFisica?.tiendDireccion || 'N/A'}</td>
                                        <td className="px-4 py-2 text-left text-base">{emp.tiendaFisica?.tiendTelefono || 'N/A'}</td>
