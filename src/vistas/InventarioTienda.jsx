...
import { Package, Boxes, Edit, Save, XCircle, PlusCircle, Trash2 } from 'lucide-react';
...
export default function InventarioTienda() {
    ...
    const fetchInventoryData = useCallback(async () => {
        ...
        try {
            const employeeResponse = await fetch(`https://lord-wine-backend.onrender.com/api/empleados/${empleadoLogeadoId}`);
            if (!employeeResponse.ok) {
                if (employeeResponse.status === 404) {
                    showNotification('No se pudo encontrar el empleado logeado.', 'error');
                } else {
            ...
            if (!employeeData.tiendIdTiendaFisica) {
                showNotification('El empleado no tiene una tienda física asignada.', 'error');
                setStoreInventory([]);
                setIsLoading(false);
                return;
            }
            ...
            const storeInventoryRelation = tieneTiendaFisicaInvTienData.find(
                rel => rel.tiendIdTiendaFisica === employeeData.tiendIdTiendaFisica
            );

            if (!storeInventoryRelation) {
                showNotification('No se encontró un inventario para la tienda del empleado.', 'error');
                setStoreInventory([]);
                setIsLoading(false);
                return;
            }

            const invTienId = storeInventoryRelation.invTienIdInventarioTienda;
            setEmpleadoInventarioTiendaId(invTienId);
            setEmpleadoTiendaNombre(employeeData.tiendaFisica.tiendNombre);

            const [productsResponse, allInventoryResponse] = await Promise.all([
                fetch('https://lord-wine-backend.onrender.com/api/productos'),
                fetch('https://lord-wine-backend.onrender.com/api/tiene-inventario-tienda-producto')
            ]);
            if (!productsResponse.ok) throw new Error('Error al obtener la lista de productos.');
            if (!allInventoryResponse.ok) throw new Error('Error al obtener datos de inventario.');

            const productsData = await productsResponse.json();
            const allInventoryData = await allInventoryResponse.json();
            setProductos(productsData);

            const filteredInventory = allInventoryData
                .filter(item => item.invTienIdInventarioTienda === invTienId)
                .map(item => ({
                    ...
                    tienda: employeeData.tiendaFisica?.tiendNombre || 'N/A',
                    updatedAt: new Date(item.updatedAt).toLocaleDateString(),
                }));
                
            setStoreInventory(filteredInventory);

        } catch (err) {
            console.error('Error al cargar datos del inventario:', err);
            showNotification('Error al cargar el inventario: ' + err.message, 'error');
        } finally {
            setIsLoading(false);
        }
    }, [empleadoLogeadoId]);
    ...
    const handleOpenModal = () => {
        if (!empleadoInventarioTiendaId) {
            showNotification('No se ha podido determinar el inventario de tu tienda.', 'error');
            return;
        }
        setIsModalOpen(true);
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            const res = await fetch(`https://lord-wine-backend.onrender.com/api/tiene-inventario-tienda-producto/${editedProduct.invTienIdInventarioTienda}/${editedProduct.prodIdProducto}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            <main className="flex-grow flex flex-col items-center w-full py-8 px-4 sm:px-8 bg-vistas-home overflow-y-auto">
                <div className="w-full mx-auto bg-white p-8 rounded-2xl shadow-lg mt-8">
                    ...
                    <p className="text-center text-black mb-8 text-xl">
                        Gestión del inventario para la tienda: <strong>{empleadoTiendaNombre}</strong>
                    </p>
